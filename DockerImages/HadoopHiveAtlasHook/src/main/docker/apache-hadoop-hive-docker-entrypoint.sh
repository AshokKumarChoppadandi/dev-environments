...
start_hiveslavenode_with_atlas_hook() {
  # STARTING HISTORY SERVER SERVICE
  echo "Starting Datanode..."
  hadoop-daemon.sh start datanode
  sleep 5
  ...
  ATLAS_PROPERTIES_FILE=${HIVE_CONFIG_DIR}/atlas-application.properties
  ...
  sed -i -e "s/ATLAS_HOSTNAME/$ATLAS_HOSTNAME/" "$ATLAS_PROPERTIES_FILE"

  sed -i -e "s/HIVE_AUX_JARS_PATH_FOR_ATLAS/$HIVE_AUX_JARS_PATH_FOR_ATLAS/" "${HIVE_CONFIG_DIR}/hive-env.sh"
